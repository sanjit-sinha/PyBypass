...
def appdrive_bypass(url: str, appdrive_email=None, appdrive_password=None) -> str:
    ...
    if ddl_btn != None:
        ...
        account_login(client, url, appdrive_email, appdrive_password)

    while data['type'] <= 3:
        try:
            response = client.post(url, data=gen_payload(data), headers=headers).json();  break
        except:
            data['type'] += 1
    ...
    if 'url' in response:
        ...
        "driveapp.in", "drivehub.in", "gdflix.pro", "drivesharer.in", "drivebit.in", "drivelinks.in", "driveace.in",
        "drivepro.in", "gdflix.top"):
            return appdrive_lookalike(client, drive_link)
        else:
            return drive_link
